...
$$\alpha' = \alpha + k$$
$$\beta' = \beta + 1$$
*Sólo falta añadirle las constantes:*
...
$$\Rightarrow p(y) = \frac{\beta^{\alpha}}{(\beta+1)^{\alpha+k}} \frac{\Gamma(\alpha+k)}{\Gamma(\alpha)k!} \int \textsf{Gamma}(\theta \, | \, \alpha', \beta') d\theta = \left(\frac{\beta}{\beta + 1}\right)^\alpha \left(\frac{1}{\beta + 1}\right)^k \frac{(\alpha+k-1)!}{(\alpha-1)!k!} \int \textsf{Gamma}(\theta \, | \, \alpha', \beta') d\theta$$
*Finalmente,*
$$\Rightarrow p(y) = {\alpha + k -1 \choose k} \left(\frac{\beta}{\beta + 1}\right)^\alpha \left(\frac{1}{\beta + 1}\right)^k \int \textsf{Gamma}(\theta \, | \, \alpha', \beta') d\theta$$
...
*Teniendo en cuenta que para calcular la posterior se usó la distribución Gamma con las reglas de actualización para una y:*
$$\alpha_1 = \alpha + y$$
$$\beta_1 = \beta + 1$$
...
$$\lambda_i=\theta_i n_i$$
donde $\theta_i$ es igual a la tasa individual de mortalidad por año en el municipio $i$ y $n_i$ la población total del municipio $i$. Al multiplicar ambas, obtenemos valor de $\lambda_i$ que representala tasa de mortalidad por neumonía en el municipio $i$
...
defunciones_tran_2019 <- defunciones_tran %>%
  mutate(muertes_totales = rowSums(defunciones_tran %>% select_if(is.numeric), na.rm = TRUE))

tasas_mort <- dplyr::full_join(demograficos,defunciones_tran_2019,by=c("entidad","municipio")) %>%
...
gamma.inicial<-rgamma(1000,exp(initial_guess[1]),exp(initial_guess[1]))
gamma.final<-rgamma(1000,params.prior[1],params.prior[1])
...
ggplot() +
geom_histogram(aes(x = gamma.final), fill = '#00BFC4', alpha = .6) +
geom_histogram(aes(x = tasas_mort$value*10000,na.rm=TRUE), alpha = .6)
```
...
```{r}
ggplot(tasas_mort,aes(x=value*10000,y=pob_total))+geom_point()+scale_y_log10()+scale_x_log10()
```
