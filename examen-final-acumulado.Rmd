...
gamma.inicial <- rgamma(1000,exp(initial_guess[1]),exp(initial_guess[2]))
gamma.final <- rgamma(1000,params.prior[1],params.prior[2])
...
geom_histogram(aes(x = tasas_mort$value*10000,na.rm=TRUE), fill = '#00BFC4', alpha = .6) +
  xlim(0,20) + ylim(0,600)
```
*Nuestra distribución a priori se ve coherente.*
...
*Utilicemos los parámetros $\alpha$ y $\beta$ recién encontrados.*
...
ggplot() +
  geom_histogram(aes(x = sim_3), fill = 'salmon', alpha = .3) +
  geom_histogram(aes(x = sim_4), fill = 'purple', alpha = .3) +
